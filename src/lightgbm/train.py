...
def oof_ridge(target_col, train_df, val_df, test_df, df_2025, feature_cols, name):
    """
    構想: OOFリッジ回帰による目的変数の残差化。

    学習データを5分割したGroupKFold OOF予測を Ridge で行い、
    目的変数からその予測値を差し引く（残差学習）。
    バリデーション・テストデータには全学習データでfitしたモデルを適用する。
    """
    ...
    joblib.dump(not_pop, f"./model/clf_{name}_input.pkl")
    ...
    for tr_idx, va_idx in gkf.split(train_df, groups=train_df['レースID']):
        ...
        mask = ~y.isna()
        clf = make_pipeline(SimpleImputer(strategy='median'), Ridge(alpha=1.0))
    ...
    clf = make_pipeline(SimpleImputer(strategy='median'), Ridge(alpha=1.0))
    clf.fit(X[mask], y[mask])
    joblib.dump(clf, f"./model/clf_{name}_model.pkl")

    for df_tgt in [val_df, test_df, df_2025]:
        df_tgt[target_col] = df_tgt[target_col].astype(float) - clf.predict(df_tgt[not_pop])
