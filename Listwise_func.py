...
def tokyo(race_id, odds):
    ...
    model.load_state_dict(torch.load("./model/tokyo_ranknet_2.pth", map_location="cuda"))
    model.eval()

    df = predict_new_data(model, df, feature_cols, embedding_cols, context_num_cols, context_cat_cols)
...
def nakayama(race_id, odds):
    ...
    model.load_state_dict(torch.load("./model/nakayama_ranknet_2.pth", map_location="cuda"))
    model.eval()
