...
def tokyo(race_id, odds):
    ...
    rank_tuner_path = f"./pickle-tuner/tokyo_rank_" # 学習済みモデルを保存する場所
    reg_tuner_path = f"./pickle-tuner/tokyo_reg_" # 学習済みモデルを保存する場所
    ...
    # 今走の処理
    df = processing.df_first_processing(df)
    # 過去走の処理
    df_all = processing.df_big_past_processing(df)
    # 過去のレベル
    df_all = processing.past_level(df_all)
    # 終了処理
    df_all = processing.df_end_processing(df_all)
    # ラベリング
    df_all = processing.encording(df_all)
